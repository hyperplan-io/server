...
class PredictionSerializerSpec
  ...
  it should "correctly encode a prediction" in {
    ...
    testEncoder(prediction) { json =>
      val expectedJson =
        s"""{"type":{"class":"Classification"},"id":"$predictionId","projectId":"$projectId","algorithmId":"$algorithmId","features":[0.0,0.0,0.5],"labels":[{"id":"$labelId","label":"","probability":0.5,"correctExampleUrl":"correct_example_url","incorrectExampleUrl":"incorrect_example_url"}],"examples":[]}"""
