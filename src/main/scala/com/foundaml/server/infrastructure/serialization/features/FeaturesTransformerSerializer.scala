...
object FeaturesTransformerSerializer {
  ...
  val tfTransformerEncoder
  : Encoder[TensorFlowFeaturesTransformer] =  (transformer: TensorFlowFeaturesTransformer) =>
    Json.obj(
      ("signatureName", Json.fromString(transformer.signatureName)),
      ("fields", Json.fromValues(transformer.fields.map(Json.fromString)))
    )

  val tfTransformerDecoder
  : Decoder[TensorFlowFeaturesTransformer] =
    (c: HCursor) =>
      for {
