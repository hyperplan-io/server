...
object PredictionSerializer {
  ...
  implicit val classificationLabelsEncoder: Encoder[Set[ClassificationLabel]] =
    Encoder.encodeSet[ClassificationLabel](ClassificationLabelSerializer.classificationlabelEncoder)

  implicit val classificationLabelsDecoder: Decoder[Set[ClassificationLabel]] =
    Decoder.decodeSet[ClassificationLabel](ClassificationLabelSerializer.classificationLabelDecoder)

  implicit val regressionLabelsEncoder: Encoder[Set[RegressionLabel]] =
    Encoder.encodeSet[RegressionLabel](RegressionLabelSerializer.regressionlabelEncoder)

  implicit val regressionLabelsDecoder: Decoder[Set[RegressionLabel]] =
    Decoder.decodeSet[RegressionLabel](RegressionLabelSerializer.regressionLabelDecoder)


  implicit val classificationPredictionEncoder
  : Encoder[ClassificationPrediction] = (prediction: ClassificationPrediction) =>
    Json.obj(
  ...
  implicit val regressionPredictionEncoder
  : Encoder[RegressionPrediction] = (prediction: RegressionPrediction) =>
    Json.obj(
      ("type", problemTypeEncoder(prediction.predictionType)),
      ("id", Json.fromString(prediction.id)),
      ("projectId", Json.fromString(prediction.projectId)),
      ...
      ("examples", Json.fromValues(prediction.examples.map(Json.fromFloatOrNull)))
    )
  ...
  : Decoder[Prediction] =
    (c: HCursor) =>
      c.downField("type").as[ProblemType](ProblemTypeSerializer.decoder).flatMap {
        case Classification() =>
          for {
          ...
          } yield ClassificationPrediction(id, projectId, algorithmId, features, examples, labels)
        case Regression() =>
          for {
            id <- c.downField("id").as[String]
            projectId <- c.downField("projectId").as[String]
