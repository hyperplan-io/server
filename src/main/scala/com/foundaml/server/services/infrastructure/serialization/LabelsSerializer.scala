...
object LabelsSerializer {
  ...
  val discriminator: Configuration = Configuration.default.withDiscriminator("labels")

  def toString(labels: Labels)(implicit encoder: Encoder[Labels]): String = {
   labels.asJson.noSpaces 
  }

  def fromString(n: String)(implicit decoder: Decoder[Labels]): Labels = {
