...
object FeaturesParserService {
    def parseFeatures(configuration: ProjectConfiguration, hcursor: Json)(implicit domainService: DomainService): IO[Features] = configuration match {
      case ClassificationConfiguration(featuresConfiguration, _) => 
        println("parsing classification features")
        parseFeatures(featuresConfiguration, hcursor.hcursor.downField("features"))
      case RegressionConfiguration(featuresConfiguration) => 
        parseFeatures(featuresConfiguration, hcursor.hcursor.downField("features"))
    }


  def parseFeatures(configuration: FeaturesConfiguration, hcursor: ACursor)(implicit domainService: DomainService): IO[Features] = {
