...
case class TensorFlowFeaturesTransformer(
  ...
  def transform(
      ...
      val tensorFlowFeatures = features.zip(fields).foldLeft(TensorFlowFeatures(signatureName, List.empty)) { case (tfFeatures, feature) =>
        feature match {
      ...
      println(s"features transformer : ${ tensorFlowFeatures.examples.mkString(",")}")
